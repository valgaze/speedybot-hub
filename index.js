...
${JSON.stringify(trigger, null, 2)}
    ...
    },
    {
        keyword: 'translate',
        handler($bot, trigger) {
            const locale = 'cn' // Chinese
            const translated = $bot.i18n(locale, 'greetings.variant2', '')
            $bot.say(translated)
        },
        helpText: 'Handler which uses i18n for content'
    }
]
...
async function SpeedybotMini(token, handlers = [], config = {
    ...
    // handler($bot, trigger)
    const bot = {
        i18n(locale, lookup, template = {}, fallback = '', ) {
...
async function handleRequest(request, event) {
    ...
    if (pass) {
        const inst = await SpeedybotMini(config.token, handlers, config)
        await inst(envelope)
        ...
        return new Response('ok', {
            status: 200,
            statusText: 'Request did not pass user-provided validation'
        })
    }
}
