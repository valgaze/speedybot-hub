...
export type HookHandler<T = any> = (
  ...
  context: Context
) => Promise<APIGatewayProxyResult | T>
...
export const SpeedyGuard = async <T = any>(
  ...
  const urlRef = new URL(
    `${event.headers['x-forwarded-proto']}://${event.requestContext.domainName}`
  )
  ...
  if (pathname in hooks) {
    ...
    if (typeof ref === 'function') {
      ref(event, context)
      return finale() as APIGatewayProxyResult
    }
    if ('handler' in ref && typeof ref.handler === 'function') {
      if ('method' in ref) {
        const { method = '' } = ref
        const actualMethod = event.requestContext.http.method
        if (method.toLowerCase() !== actualMethod.toLowerCase()) {
          return buildLambdaResponse(
            `Expected method '${method}' rather than '${actualMethod}'`,
            400
          )
        }
      }
      ...
      if ('validate' in ref && typeof ref.validate === 'function') {
        const { proceed } = await ref.validate(event, context)
        if (proceed) {
          return ref.handler(event, context) as any
        } else {
          return buildLambdaResponse('Validation failed', 400)
        }
      }

      await ref.handler(event, context)
      return finale() as APIGatewayProxyResult
    }
  }
